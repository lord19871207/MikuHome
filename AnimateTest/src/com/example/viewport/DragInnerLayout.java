 ...
 * 
 * 2013年谷歌i/o大会上介绍了两个新的layout： SlidingPaneLayout 和DrawerLayout， 现在这俩个类被广泛的运用，其实研究他们的源码你会 发现这两个类都运用了ViewDragHelper来处理拖动。
 * ViewDragHelper是framework中不为人知却非常有用的一个工具。
 * 
 * 其实ViewDragHelper并不是第一个用于分析手势处理的类，gesturedetector也是， 但是在和拖动相关的手势分析方面gesturedetector只能说是勉为其难。关于ViewDragHelper有如下几点：
 * ViewDragHelper.Callback是连接ViewDragHelper与view之间的桥梁（ 这个view一般是指拥子view的容器即parentView）；ViewDragHelper的实例是通过静态工厂方法创建的；
 * 你能够指定拖动的方向；ViewDragHelper可以检测到是否触及到边缘； ViewDragHelper并不是直接作用于要被拖动的View，而是使其控制的视图容器中的
 * 子View可以被拖动，如果要指定某个子view的行为，需要在Callback中想办法； ViewDragHelper的本质其实是分析onInterceptTouchEvent和onTouchEvent
 * 的MotionEvent参数，然后根据分析的结果去改变一个容器中被拖动子View的位置（ 通过offsetTopAndBottom(int offset)和offsetLeftAndRight(int offset) 方法
 * ），他能在触摸的时候判断当前拖动的是哪个子View； 虽然ViewDragHelper的实例方法 ViewDragHelper create(ViewGroup forParent, Callback cb)
 * 可以指定一个被ViewDragHelper处理拖动事件的对象 ，但ViewDragHelper类的设计决定了其适用于被包含在一个自定义 ViewGroup之中，而不是对任意一个布局上的视图容器使用ViewDragHelper。
 * 
 * @Package com.example.viewport
